from flask import Flask, request, jsonify
from flask_mysqldb import MySQL
import logging
from logging.handlers import TimedRotatingFileHandler

logger = logging.getLogger(__name__)

logging.basicConfig(
    level=logging.INFO,
    format='[%(asctime)s] %(levelname)s in %(module)s: %(funcName)s : %(lineno)d : %(message)s',
    handlers=[
        logging.handlers.TimedRotatingFileHandler(
            filename=r"Runtime_log.log",
            when="D",
            interval=1,
            backupCount=0,
        )
    ]
)

# Create a SQL Instance
mysql = MySQL()

# Create a Flask Instance
app = Flask(__name__)

app.config['SECRET_KEY'] = "this is my secret key"

# Configurations for the MySQL Connection
app.config['MYSQL_HOST'] = "localhost"
app.config["MYSQL_USER"] = "root"
app.config["MYSQL_PASSWORD"] = "Password"
app.config["MYSQL_DB"] = "Students_DB"

# Initialize MySQL
mysql.init_app(app)


# Function to Insert Student Records
@app.route('/create', methods=['POST'])
def add_student():
    try:
        app.logger.info('Student Insert Process Starting')
        student_name = request.json["student_name"]
        dob = request.json["dob"]
        email = request.json["email"]
        mobile_no = request.json["mobile_no"]
        gender = request.json["gender"]
        course = request.json["course"]

        con = mysql.connect
        cur = con.cursor()

        cur.callproc("create_student", (student_name, dob, email, mobile_no, gender, course))
        app.logger.info('Student Insert request received Successfully')

        con.commit()
        con.close()
        values = request.get_json()

        app.logger.info('Student Records Inserted Successfully')
        return jsonify(values, "Data Successfully Uploaded")

    except Exception as e:
        app.logger.error('An error occurred: %s', str(e))
        return jsonify(e, "An ERROR occurred in Add Student Method")


# Function to get all the Students Records
@app.route('/students', methods=['GET'])
def view_student():
    try:
        app.logger.info("Get All Student Records Process Starting")

        con = mysql.connect
        cur = con.cursor()

        cur.callproc("view_student")
        results = cur.fetchall()
        app.logger.info('Get All request received Successfully')

        # response = json.loads(results)
        return jsonify(results)
        con.commit()
        con.close()

    except Exception as e:
        app.logger.error('An error occurred: %s', str(e))
        return jsonify(e, "An ERROR occurred in View Student Method")


# Function to Update the Particular Student Records
@app.route('/update', methods=['PUT'])
def update():
    try:
        app.logger.info('Update Process Starting')

        id = request.json["id"]
        s_name = request.json["student_name"]
        email = request.json["email"]
        m_no = request.json["mobile_no"]
        course = request.json["course"]

        con = mysql.connect
        cur = con.cursor()
        cur.callproc("get_student", (id,))
        row = cur.fetchone()

        app.logger.info('Checking the ID existence')
        if row is None:
            app.logger.info('Record Not Found to Update For This Specific ID')
            return jsonify("Record not found"), 404

        con = mysql.connect
        cur = con.cursor()
        cur.callproc("update_student", (id, s_name, email, m_no, course))
        app.logger.info('Student Update Request Received Successfully')
        con.commit()
        values = request.get_json()

        app.logger.info('Update Process Successfully Done')
        return jsonify(values,'Student Record has updated successfully')
        con.close()

    except Exception as e:
        app.logger.error('An error occurred: %s', str(e))
        return jsonify(e, "An ERROR occurred in Update Student Method")


# Function to Delete the Particular Student Record
@app.route('/delete/<int:id>', methods=['DELETE'])
def delete(id):
    try:
        app.logger.info('Delete Method Starting')
        con = mysql.connect
        cur = con.cursor()
        cur.callproc("get_student", (id,))
        row = cur.fetchone()

        app.logger.info('Checking the ID existence')
        if row is None:
            app.logger.info('Record Not Found to Update For This Specific ID')
            return jsonify("Record not found"), 404

        con = mysql.connect
        cur = con.cursor()
        cur.callproc("delete_student", (id,))
        con.commit()

        app.logger.info('Student Delete Process Successfully Done')
        con.close()
        return jsonify('Student Record has deleted successfully')

    except Exception as e:
        app.logger.error('An error occurred: %s', str(e))
        return jsonify(e, "An ERROR occurred Delete Method")


if __name__ == '__main__':
    app.run(debug=True)

